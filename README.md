...
## provides a list of all earthquakes happening around the globe using USGS API earthquakes can be categorized on the basis of:
cordinates
date of occurence
magnitude
month/year
...
Code is easy to follow, logically. Standards of indentation and formatting are followed, so that the code and its structure are clearly visible. Variables are named meaningfully, so that they communicate intent.
